...
export default function Team({ team }: Props) {
    ...
    return (
       <TableRow hover={true} sx={rowStyle} key={team.team.name}>
           ...
           <TableCell sx={cellStyle} align="center">{team.gamesPlayed}</TableCell>
           <TableCell sx={cellStyle} align="center">{team.leagueRecord.wins}</TableCell>
           <TableCell sx={cellStyle} align="center">{team.leagueRecord.losses}</TableCell>
           <TableCell sx={cellStyle} align="center">{team.leagueRecord.ot}</TableCell>
           <TableCell sx={[cellStyle, ptsCol]} align="center">{team.points}</TableCell>
           <TableCell sx={cellStyle} align="center">{(team.pointsPercentage).toFixed(3)}</TableCell>
           <TableCell sx={cellStyle} align="center">{team.regulationWins}</TableCell>
           <TableCell sx={cellStyle} align="center">{team.row}</TableCell>
           <TableCell sx={cellStyle} align="center">{team.goalsScored}</TableCell>
           <TableCell sx={cellStyle} align="center">{team.goalsAgainst}</TableCell>
           <TableCell sx={cellStyle} align="center" 
           className={`${goalDiff > 0 ? 'positive-diff' : goalDiff < 0 ? 'negative-diff' : ''}`}
           >{goalDiff}</TableCell>
           <TableCell sx={cellStyle} align="center">{team.streak.streakCode}</TableCell>
       </TableRow>
    )
}